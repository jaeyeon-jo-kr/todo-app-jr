...
FROM json_populate_recordset(null::posts,
'[
   {
     "user_id": 1,
     "id": 1,
...
FROM json_populate_recordset(null::comments,
'[
{
  "post_id": 1,
  "id": 1,
...
DROP TYPE IF EXISTS geo_type CASCADE;
CREATE TYPE geo_type AS(
    lat FLOAT,
    lng FLOAT
);
...
SELECT *
INTO TEMPORARY users_temp
FROM json_populate_recordset(null::user_type,
'[
   {
     "id": 1,
     "name": "Leanne Graham",
...
SELECT ((address::address_type).geo::geo_type).lat,
((address::address_type).geo::geo_type).lng
FROM users_temp;

INSERT INTO Addresses(street, suite, city, zipcode, geo_id)
SELECT (address::address_type).street,
(address::address_type).suite,
(address::address_type).city,
(address::address_type).zipcode,
g.id
FROM users_temp AS u
INNER JOIN Geos AS g
ON g.lat = ((u.address::address_type).geo::geo_type).lat
AND g.lng = ((u.address::address_type).geo::geo_type).lng;

INSERT INTO Companies(name, catch_phrase, bs)
SELECT DISTINCT (company::company_type).name,
(company::company_type).catch_phrase,
(company::company_type).bs
FROM users_temp;

INSERT INTO Users (id, name, username, email, address_id, phone, website, company_id)
SELECT u.id, u.name, u.username, u.email, a.id, u.phone, u.website, c.id
FROM users_temp AS u
INNER JOIN Addresses a
ON a.street = (u.address::address_type).street
AND a.suite = (u.address::address_type).suite
AND a.city = (u.address::address_type).city
AND a.zipcode = (u.address::address_type).zipcode
INNER JOIN Companies c
ON c.name = (u.company::company_type).name;